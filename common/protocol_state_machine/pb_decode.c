...
{    
    ...
    /* Allocate new or expand hid_old allocation */
    ...
    while (stream->bytes_left)
    {
            ...
            && iter.pSize == iter.pData)
        {
            if (fixed_count_field != iter.pos) {
                /* If the new fixed count field does not match the hid_old one,
                 * check that the hid_old one is NULL or that it finished
